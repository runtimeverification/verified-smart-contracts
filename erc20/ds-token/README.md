...
* *Safe checks for overflow and underflow*: For both `transfer` and `transferFrom` it checks for underflow and for overflow, throwing an exception immediately.
...
We took the [token source code][token], [base token source code][base] and the additional libraries [ds-auth], [ds-math], [ds-stop] and [ds-note], and compiled it to the EVM bytecode using solc compiler (of the version `0.4.18+commit.9cf6e910.Linux.g++`).
