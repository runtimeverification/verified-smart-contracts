...
The DSToken ERC20 token is a high-profile ERC20 token contract written in Solidity by [DappHub](https://dapphub.com/).

We found that the DSToken ERC20 token deviates from the [ERC20-K] (and thus [ERC20-EVM]) specification as follows:

* *Stoppable modifier*: The `transfer`, `transferFrom` and `approve` functions are defined with the `stoppable` modifier
  (inherited from [`DSStop`](https://github.com/dapphub/ds-stop/blob/32fffeb2142cb34b71d8a7914f784244b44d1ede/src/stop.sol#L21-L24)).
  The `stoppable` modifier ensures that the function will throw an exception if the [`stopped`](https://github.com/dapphub/ds-stop/blob/32fffeb2142cb34b71d8a7914f784244b44d1ede/src/stop.sol#L19) variable is set to true by authorized users.
...
at commit [`bb98ff`][version]:

* [token.sol][token] - inheriting [base.sol][base]
