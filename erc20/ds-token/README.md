...
* *Stoppable modifier*: It uses the stoppable modifier from `ds-stop`, which if it is set to true by the owner of the token it throws exception in `transfer`, `transferFrom` and `approve`. 
...
https://github.com/dapphub/ds-token/tree/bb98ff4841e82137efc9690b9e1cf77e85fdd0bb,
commit [`1a1a5b6`][version]:
* [token.sol][token]
* [base.sol][base]
...
* To capture the exception in `transfer`, `transferFrom` and `approve`, we added the following variable in `storage`
```
    #hashedLocation({COMPILER}, {_STOPPED}, .IntList) |-> STOPPED
```

	We have added also in `required` of the succes case:
